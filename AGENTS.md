...
## 2. CodeGraph 代码图谱工具规范

CodeGraph 为代码库构建语义知识图谱, 支持快速的符号查询与代码关系分析。

### 2.1 会话启动自检(强制)

- 必须先定位仓库根目录, 禁止基于当前工作目录直接判断是否初始化。
- 根目录获取方式: `git rev-parse --show-toplevel`。
- CodeGraph 状态判定流程:
    1. 检查 `<repo_root>/.codegraph/` 是否存在。
    2. 若存在, 必须调用 `codegraph_status(projectPath="<repo_root>")` 进行状态确认。
    3. 若状态确认成功, 判定为已初始化, 后续所有 CodeGraph 工具调用都必须显式携带 `projectPath="<repo_root>"`。
    4. 若目录存在但状态确认失败, 视为索引异常, 向用户说明后建议执行 `codegraph init -i` 重建。
    5. 仅当 `<repo_root>/.codegraph/` 不存在时, 才判定为未初始化。

### 2.2 工具可用时的使用规范(`.codegraph/` 存在且状态正常)

当项目已初始化 CodeGraph 时, 优先使用以下工具进行代码探索, 而不是传统文本搜索:

| 工具                | 用途                                 |
| ------------------- | ------------------------------------ |
| `codegraph_search`  | 按名称查找符号(函数、结构体、类型等) |
| `codegraph_context` | 获取任务相关的代码上下文             |
| `codegraph_callers` | 查找调用某函数的所有位置             |
| `codegraph_callees` | 查找某函数调用的所有被调函数         |
| `codegraph_impact`  | 分析修改某符号的影响范围             |
| `codegraph_node`    | 获取符号的详细信息与完整源代码       |

**快速查询建议:**

- 查找符号定义或引用时, 使用 `codegraph_search` 代替 grep。
- 追踪代码流程时, 优先使用 `codegraph_callers`/`codegraph_callees`。
- 修改前必须用 `codegraph_impact` 评估关联范围。
- 所有 CodeGraph 查询都必须传入 `projectPath="<repo_root>"`, 禁止省略。
- 强制要求: 即使当前会话已经位于仓库根目录, 也必须显式传 `projectPath`。
- 推荐写法: `codegraph_status(projectPath="<repo_root>")`, `codegraph_context(task="...", projectPath="<repo_root>")`。

**生成子任务或委派探索时:**

- 明确告知 AI 工具使用 CodeGraph 工具进行快速探索。

### 2.3 工具不可用时的初始化(`.codegraph/` 不存在)

会话开始时, 若项目未初始化 CodeGraph, 应向用户建议初始化:

> 我检测到本项目未初始化 CodeGraph。是否运行 `codegraph init -i` 来构建代码知识图谱? 这将显著提升后续代码探索与分析效率。
