...
REPORT_PATH = Path('plans/tao-codec_vorbis_coverage/tao-codec_vorbis_samples_report.md')
...
示例:
  ...
  # 只测试指定序号(可多个)
  python plans/tao-codec_vorbis_coverage/run_vorbois_samples_compare.py --index 3 5 8

  # 指定并行数量
  python plans/tao-codec_vorbis_coverage/run_vorbois_samples_compare.py --jobs 4
        """,
    )
...
def main():
    ...
    print(f'共 {len(pending)} 条记录待处理, 并行数: {jobs}, 单样本超时: {args.timeout}s', flush=True)
