...
REPORT_PATH = Path("plans/tao-codec/audio/wav/coverage/decoder_report.md")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="WAV 解码器样本批量对比工具, 从项目根目录运行.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 默认断点续测(只处理尚未测试的记录)
  python plans/tao-codec/audio/wav/coverage/run_decoder.py

  # 重新测试所有精度不为 100%% 的记录(含失败)
  python plans/tao-codec/audio/wav/coverage/run_decoder.py --retest-imprecise

  # 只重新测试失败的记录
  python plans/tao-codec/audio/wav/coverage/run_decoder.py --retest-failed

  # 重新测试全部记录
  python plans/tao-codec/audio/wav/coverage/run_decoder.py --retest-all

  # 只测试指定序号(可多个)
  python plans/tao-codec/audio/wav/coverage/run_decoder.py --index 3 5 8

  # 指定并行数量
  python plans/tao-codec/audio/wav/coverage/run_decoder.py --jobs 4
        """,
    )
