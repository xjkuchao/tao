...
- data/2.mp4 (Main CABAC + B帧) Frame 0 bit-exact, Frame 1 (P) 99.56%, B帧大幅偏差
  ...
  - **首个误差 MB**: MB(109,9) = P_L0_L0_8x16, 右分区 MVD_y 解码错误 (-12, 应为 -16)
  - **确认**: CABAC bypass 位在右分区 MVD 的 Exp-Golomb 后缀部分产生错误值
  - **确认**: 存在至少两个独立误差源 (MB(109,9) 和 MB(83,39)), 非级联导致
  - **确认**: 强制正确 MVD 后 MB(109,9) 像素正确, 但 MB(83,39) 等误差仍存在
  - 已排除: 加权预测, qpel 插值, 去块, CBP, CABAC 初始化表, 参考帧, MV预测
  - 已确认正确: mb_skip_flag 上下文, mb_type 解码, ref_idx 解码, MVD 编解码逻辑
  - **根因推断**: CABAC 内部状态 (range/offset) 在某处产生微小偏移, 导致后续 bypass 位解码错误
  - **下一步**: 需要逐 bin 对比 CABAC 状态来精确定位首个偏移点
