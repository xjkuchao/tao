...
- [ ] SPS/PPS 完整解析与合法性校验。
    ...
    - [x] `parse_avcc_config` 增加 avcC 保留位合法性校验(`lengthSizeMinusOne/numOfSPS` reserved bits), 并补齐失败单测。
...
- [ ] 残差逆变换/反量化覆盖 4x4/8x8 关键路径。
    - [x] I_NxN `transform_size_8x8_flag` 路径升级为规范 8x8 残差实现: 移除 4x4 近似拆分与临时开关, 接入 `dequant_8x8_ac + idct_8x8 + apply_8x8_ac_residual`, 并补 8x8 残差单测。
...
- [ ] 去块滤波。
    ...
    - [x] 宏块边界 `bs` 增加 `ref_idx_l0 + mv_l0` 运动一致性判定, 支持“同参考且运动接近时 `bs=0` 跳过滤波”, 并补专项单测。
- [ ] 容错与边界保护(坏 NAL、缺参考帧、越界)。
    ...
    - [x] 补齐坏 NAL 容错: `forbidden_zero_bit` 非法 NAL 直接拒绝, 并对“无有效 NAL 包/slice header 解析失败”记录丢弃计数与限流告警, 补齐对应单测。
    - [x] 补齐 slice 解码边界容错: 参数集激活失败、CABAC 起始越界、CAVLC bit 偏移越界统一记录坏 NAL 丢弃计数, 并补齐对应单测。
    - [x] 补齐 `first_mb` 越界容错: CABAC/CAVLC 路径统一记录坏 NAL 丢弃计数, 并补齐对应单测。
