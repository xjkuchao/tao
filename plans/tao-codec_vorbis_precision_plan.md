...
- [x] 记录 Tao/FFmpeg 样本数差异与可能的固定偏移量。
  - 当前状态: 严格口径精度已达 `100.00%`, `best_shift=0`, 样本数 `Tao=881998`, `FFmpeg=881996`(尾部多 2 样本)。
- 产出: 基线日志与问题画像。

### P1 对比链路一致性校验
- [x] 校验 Tao 与 FFmpeg 选取的音频流一致(流索引/采样率/声道数)。
- [x] 校验输出样本格式一致(`f32le`, 交错布局)。
- [x] 若存在样本差异, 先应用可解释的对齐策略验证上限。
- 产出: 对齐前后对比报告。
...
- [x] `data/1.ogg` 精度达到 `100.00%`(严格口径, 不使用容错口径)。
  - 最新结果: `max_err=0.000001`, `psnr=145.66dB`, `精度=100.00%`, `Tao/lewton=100.00%`。
- [x] 保留最终基线日志与关键实现说明。
- 产出: 验收记录与后续扩样建议。
